...
const getUser = ({
  ...
  let q = `query{ user(${param}) {ID user_email user_pass first_name last_name}}`;
  return q;
}
...
const AuthAPI = {
  login(user_email, password, request) {
    ...
    return graphql(getUserByEmail(user_email)).then((data) => {
      let user = data.user;
      var error = null;
      if (user == null) {
...
//   // loginPassport(req, res, next, passport) {
...
//     },
//     (email, password, done) => {
//       graphql(getUserByEmail(email)).then((data) => {
//         let user = data.user;
//         var error = null;
//         if (user == null) {
//           error = LoginErr.USER_NOT_EXIST
//         } else {
//           try {
//             isValid = bcrypt.compareSync(password, user.user_pass)
//             if (!isValid) {
//               error = LoginErr.INVALID
//             }
//           } catch (err) {
//             error = LoginErr.NOT_BYCRYPT_HASH
//           }
//         }
//         if (error == null) {
