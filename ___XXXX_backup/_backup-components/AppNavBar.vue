...
<div>
  ...
  <div v-if="!isProd && IsDevToolEnable" class="text-left">
    <b>[DEV ONLY] Click button below to toggle enable/disable tab</b>
    <div :style="{ display: 'flex', marginBottom: '17px'}" >
      <button 
          :class="`btn-toggle ${isTabDisabled(d.id) ? 'btn-toggle-disabled' : ''}`" 
          v-for="(d,i) in data" :key="d.id"
          @click="toogleTab(d.id)">
          {{d.id}}
        </button>
    </div>
  </div>
  ...
  <div v-if="IsOfflineEnable && getIsModeOffline()" class="jpn-offline">
    <i v-for="(d,i) in [], 3" :key="i" class="fa fa-star jo-star"></i>
    <span>URUSNIAGA DI LUAR TALIAN</span> 
    <i v-for="(d,i) in [], 3" :key="i" class="fa fa-star jo-star"></i>
    <span>URUSNIAGA DI LUAR TALIAN</span> 
    <i v-for="(d,i) in [], 3" :key="i" class="fa fa-star jo-star"></i>
  </div>
...
import { HelpUrl, IsOfflineEnable, isProd, IsDevToolEnable } from "../config/app-config";
...
export default {
  ...
  data() {
    return {
      IsDevToolEnable : IsDevToolEnable,
      isProd: isProd,
