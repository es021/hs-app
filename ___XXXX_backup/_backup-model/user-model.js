...
function getSearchWorkAvailability(field, av_month, av_year) {
  ...
  return `CONCAT((${selectMetaMain(field, UserMeta.AVAILABLE_YEAR)}),
                (${selectMetaMain(field, UserMeta.AVAILABLE_MONTH)}))
                <= '${av_year}${av_month}'`;
}

function getSearchLookingFor(field, search_params) {
  return getSearchMeta(field, search_params, UserMeta.LOOKING_FOR);
}

function getSearchUniversity(field, search_params) {
  return getSearchMeta(field, search_params, UserMeta.UNIVERSITY);
}

function getSearchMajor(field, search_params) {
  return getSearchMeta(field, search_params, UserMeta.MAJOR);
}

function getSearchStudyPlace(field, search_params) {
  return getSearchMeta(field, search_params, UserMeta.STUDY_PLACE);
}

function getSearchMinor(field, search_params) {
  return getSearchMeta(field, search_params, UserMeta.MINOR);
}

function getSearchName(field, search_params) {
  return `CONCAT((${selectMetaMain(field, UserMeta.FIRST_NAME)}),
                (${selectMetaMain(field, UserMeta.LAST_NAME)}))
                like '%${search_params}%'`;
}

function getSearchEmail(field, search_params) {
  return `(${selectUserField(field, User.EMAIL)}) like '%${search_params}%'`;
}
...
function getSearchQuery(params) {
  ...
  var name = (typeof params.search_user === "undefined") ? "" :
    `CONCAT((${selectMetaMain("u.ID", UserMeta.FIRST_NAME)}),' ',
            (${selectMetaMain("u.ID", UserMeta.LAST_NAME)}))
  ...
  if (typeof params.has_feedback !== "undefined" && params.has_feedback) {
    var feedbackMeta = `(${selectMetaMain("u.ID", UserMeta.FEEDBACK)})`;
    query += `and (${feedbackMeta} != '' AND ${feedbackMeta} IS NOT NULL)`;
  }

  // search degree
  query += (typeof params.search_degree === "undefined") ? "" :
    ` and CONCAT((${selectMetaMain("u.ID", UserMeta.MAJOR)}),
            (${selectMetaMain("u.ID", UserMeta.MINOR)}))
            like '%${params.search_degree}%'`;

  // search university
  query += (typeof params.search_university === "undefined") ? "" :
    ` and (${selectMetaMain("u.ID", UserMeta.UNIVERSITY)}) like '%${params.search_university}%'`;
...
function getUser(field, params, meta_cons) {
  ...
  var role_condition = (typeof params.role !== "undefined") ? `(${selectMetaMain("u.ID", UserMeta.ROLE)}) LIKE '%${params.role}%' ` : `1=1`;
  ...
  for (var k in UserMeta) {
    ...
    if (typeof field[meta_key] !== "undefined") {
      meta_sel += `, ${selectMeta("u.ID", UserMeta[k], meta_key)}`;
    }
  }
